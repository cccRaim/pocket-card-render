...
> **需要 AssetStudio 吗?正常情况不需要。** 几何来自 AssetRipper,材质来自 UnityPy——AssetStudio 的
> CLI 连 `Material` 都 dump 不了(`-m dump` 只产出 Mesh / Texture2D / MonoBehaviour / Shader)。它**唯一**
> 的用处是**着色器字节码提取**,用于逆向一个*新*着色器:*Export Raw* → 着色器的 SMOL-V blob → SPIR-V →
> SPIRV-Cross → GLSL。本仓库里那些字节追踪出来的着色器就是这么来的;只有新增全新效果时才需要它
> (见 CONTRIBUTING.md)。
