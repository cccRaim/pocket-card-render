...
import { DYNAMIC_UNIFORM_PRODUCER_SCHEMA } from "../public/render/dynamic-uniform-producer.js";
import { unityTexEnvToThreeGltfSt } from "../public/render/texture-transform.js";
import { prepareWebglVertexSource } from "../public/render/webgl-position-invariance.js";
...
function loadContext(field, selectorKey, session) {
  ...
  assert.equal(port.obligations?.[field]?.verifier, `build/verify-official-port-${field
    ...
    .replace("commonBindings", "common-bindings")
    .replace("runtimeDispatch", "runtime-dispatch")}.mjs`);
  const runtimeContract = manifest.runtime_contract
    ? compileWebglRuntimePortContract(manifest.runtime_contract, {
...
export function verifyDynamicUniformBindings({
  contract,
  producerContract,
  materialUniforms,
  programUniforms,
  ...
  const errors = [];
  const dynamicUniforms = contract || {};
  if (Object.keys(dynamicUniforms).length === 0) return errors;
  if (producerContract?.schema !== DYNAMIC_UNIFORM_PRODUCER_SCHEMA
      || !sameJson(producerContract?.uniforms, dynamicUniforms)) {
    errors.push(`${prefix}: dynamic uniform producer contract mismatch`);
  }
  for (const [name, spec] of Object.entries(dynamicUniforms)) {
    const uniform = programUniforms.get(name);
    const expectedType = parseWebglRuntimeUniformType(spec.type);
    if (uniform?.type !== WEBGL_TYPES.get(expectedType.baseType)
        || Number(uniform?.size) !== expectedType.size) {
      errors.push(`${prefix}: ${name} dynamic uniform type mismatch`);
      continue;
    }
    if (!sameJson(uniform.value, materialUniforms?.[name])) {
      errors.push(`${prefix}: ${name} material-to-program upload mismatch`);
    }
    if (spec.source === "official-clock"
        && (!Number.isFinite(Number(officialTime)) || Number(uniform.value) !== Number(officialTime))) {
      errors.push(`${prefix}: ${name} does not match OfficialClock`);
    }
  }
  return errors;
}
...
function verifyManifestStageProgram(context) {
  ...
  const compiledAdaptation = compileWebglAdaptationContract(adaptation);
  assert.equal(compiledAdaptation.sourceSchema, WEBGL_ADAPTATION_SCHEMA_V2);
  assert.equal(adaptation.operationGraphSha256, compiledAdaptation.operationGraphSha256);
...
function genericRuntimeDraws(context) {
  ...
  const contract = context.runtimeContract;
  ...
  for (const canonical of CANONICAL_FULL_RUNTIME_SCENES) {
    ...
    for (const { prefix, draw, recipe, expected } of runtimeMatch.draws) {
      ...
      draws.push({ prefix, draw, recipe, expected, capture, scene });
    }
  }
...
function verifyGenericRuntimePort(field, context) {
  ...
  const rendererPropertyBlock = contract.renderer_uniforms
    ? compileRendererPropertyBlockContract(contract.renderer_uniforms)
    : null;
  ...
  for (const { prefix, draw, recipe, capture, scene } of runtime.draws) {
    ...
    const resolvedStencilRegion = resolveStencilRegionFloats(recipe, pass);
    const stateRecipe = resolvedStencilRegion.binding
      ? { ...recipe, floats: resolvedStencilRegion.floats }
      : recipe;
    if (!sameJson(
      draw.material?.stencilRegionBinding || null,
      resolvedStencilRegion.binding || null,
    )) {
      passErrors.push(`${prefix}: stencil region binding provenance mismatch`);
    }
    const cull = resolveOfficialParameter(pass.culling, stateRecipe, defaults);
    ...
    const zTest = resolveOfficialParameter(pass.depth.test, stateRecipe, defaults);
    const zWrite = resolveOfficialParameter(pass.depth.write, stateRecipe, defaults) !== 0;
    ...
    if (!blend.enabled
        || blend.srcRgb !== UNITY_BLEND_FACTORS.get(resolveOfficialParameter(expectedBlend.src_rgb, stateRecipe, defaults))
        || blend.dstRgb !== UNITY_BLEND_FACTORS.get(resolveOfficialParameter(expectedBlend.dst_rgb, stateRecipe, defaults))
        || blend.srcAlpha !== UNITY_BLEND_FACTORS.get(resolveOfficialParameter(expectedBlend.src_alpha, stateRecipe, defaults))
        || blend.dstAlpha !== UNITY_BLEND_FACTORS.get(resolveOfficialParameter(expectedBlend.dst_alpha, stateRecipe, defaults))
        || blend.equationRgb !== UNITY_BLEND_OPS.get(resolveOfficialParameter(expectedBlend.op_rgb, stateRecipe, defaults))
        || blend.equationAlpha !== UNITY_BLEND_OPS.get(resolveOfficialParameter(expectedBlend.op_alpha, stateRecipe, defaults))
        || !sameJson(blend.colorMask, colorMask(resolveOfficialParameter(expectedBlend.color_mask, stateRecipe, defaults)))) {
      passErrors.push(`${prefix}: blend state mismatch`);
    }
    const stencilComp = resolveOfficialParameter(pass.stencil.generic.comp, stateRecipe, defaults);
    const stencilOps = ["fail", "zFail", "pass"].map((name) => resolveOfficialParameter(pass.stencil.generic[name], stateRecipe, defaults));
    ...
    if (canDisableStencil) {
      ...
      const ref = resolveOfficialParameter(pass.stencil.ref, stateRecipe, defaults);
      const readMask = resolveOfficialParameter(pass.stencil.read_mask, stateRecipe, defaults);
      const writeMask = resolveOfficialParameter(pass.stencil.write_mask, stateRecipe, defaults);
      assert.equal(contract.stencil_face_mode, "generic");
      for (const face of ["front", "back"]) {
        const actual = stencil[face] || {};
        const source = pass.stencil.generic;
        if (actual.func !== UNITY_COMPARE.get(resolveOfficialParameter(source.comp, stateRecipe, defaults))
            || actual.ref !== ref || actual.valueMask !== readMask || actual.writeMask !== writeMask
            || actual.fail !== UNITY_STENCIL_OP.get(resolveOfficialParameter(source.fail, stateRecipe, defaults))
            || actual.depthFail !== UNITY_STENCIL_OP.get(resolveOfficialParameter(source.zFail, stateRecipe, defaults))
            || actual.pass !== UNITY_STENCIL_OP.get(resolveOfficialParameter(source.pass, stateRecipe, defaults))) {
    ...
    for (const transform of contract.texture_coordinates?.vertex?.transforms || []) {
      const uniform = uniforms.get(transform.uniform);
      let expected = null;
      try {
        expected = unityTexEnvToThreeGltfSt(
          recipe?.textures?.[transform.slot],
          manifest.official_shader_property_defaults?.textureDescriptors?.[transform.slot],
        );
      } catch {
        bindingErrors.push(`${prefix}: ${transform.uniform} serialized TexEnv is unresolved`);
      }
      if (uniform?.type !== WEBGL_TYPES.get("vec4") || !expected
          || !sameJson(uniform?.value, expected)) {
        bindingErrors.push(`${prefix}: ${transform.uniform} texture-coordinate binding mismatch`);
      }
    }
    bindingErrors.push(...verifyDynamicUniformBindings({
      contract: contract.dynamic_uniforms,
      producerContract: draw.material?.dynamicUniformProducerContract,
      materialUniforms: draw.uniforms,
      programUniforms: uniforms,
      officialTime: capture?.transformProbe?.neutral?.officialTime,
      prefix,
    }));
    ...
    if (rendererPropertyBlock) {
      bindingErrors.push(...verifyKiraPuyoRendererPropertyBlock({
        contract: rendererPropertyBlock,
        recipe,
        runtimeSettings: scene.runtimeSettings,
        producerAudit: draw.material?.rendererPropertyBlockAudit,
        materialUniforms: draw.uniforms,
        programUniforms: uniforms,
        prefix,
      }));
    ...
    for (const row of samplerBindings) {
      ...
      const backendDefault = contract.backend_texture_defaults?.[row.slot];
      if (!recipeTexture?.url && backendDefault
          && sampler?.materialTexture?.backendTextureDefault !== backendDefault) {
        bindingErrors.push(`${prefix}: ${row.slot} backend texture default mismatch`);
      }
    }
    ...
    if (contract.require_complete_active_bindings) {
      ...
      const expectedUniforms = new Set([
        ...
        ...(contract.texture_coordinates?.vertex?.transforms || []).map((row) => row.uniform),
        ...Object.keys(rendererPropertyBlock?.values || {}),
