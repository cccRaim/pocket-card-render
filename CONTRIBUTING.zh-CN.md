...
渲染器由官方材质数据和 Shader bytecode 驱动。证据覆盖率不等于官方运行时一致性或视觉一致性；定义见
[游戏渲染还原度](FIDELITY.zh-CN.md)。没有拍脑袋的魔法数字。两条规则:
...
一次不引入回归的改动会让你没碰到的卡保持**像素一致**(SR 卡没有动画层 → diff 应为 0%;UR 闪粉 + CJK 文本
抗锯齿会产生不到 1% 的差异,那是时序而非逻辑)。这只能验证相对本渲染器旧输出没有回归，不能证明与游戏
官方画面一致。用 `?only=<材质名>` 单独看某层,`?nohud` 出干净截图。
